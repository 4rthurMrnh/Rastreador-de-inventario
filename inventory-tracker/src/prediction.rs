...
pub fn regressao_linear(historico: &[VendaHistorico]) -> Result<f64, PrevisaoErro> {
    ...
    let x_data: Vec<f64> = (0..historico.len()).map(|i| i as f64).colect();
    ...
    let records = Array2::from_shape_vec((historico.len(), 1), x_data)
        .map_err(|e| Previsao::ErroTreinamento(format!("Erro ao criar Array X: {}", e)))?;
    ...
    let previsao = modelo.predict(previsao_records);
